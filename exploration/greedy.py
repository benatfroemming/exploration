...
class HyperParams:
    """No additional exploration hyperparameters."""
    pass


class GreedyAgent:
    """
    DQN agent with purely greedy action selection (epsilon = 0).
    Useful for fine-tuning a pre-trained checkpoint or as a baseline.
    """
    ...
    def train(self, env, num_episodes: int, log_path: str, model_dir: str) -> None:
        ...
        for episode in range(1, num_episodes + 1):
            ...
            while ep_len <= self.hp.MAX_EPISODE_LENGTH:
                ...
                if len(self.replay_buffer) >= self.hp.MIN_BUFFER_SIZE:
                    ...
                    if self.total_env_steps % self.hp.TARGET_UPDATE == 0:
                        self._sync_target()
                        print(f"  [target sync] step={self.total_env_steps:,}  ε=0.0000")
            ...
            record = {
                ...
                "ep_len":      ep_len,
                "epsilon":     0.0,          # always 0 — kept for log compatibility
            }
            ...
            if self.total_env_steps > 0 and self.total_env_steps % 2_000_000 == 0:
                ckpt = os.path.join(model_dir, f"{self._model_stem()}_step{self.total_env_steps}.pth")
                self.save(ckpt)

...
class UCBAgent:
    ...
    def select_action(self, state_tensor: torch.Tensor) -> tuple[int, float]:
        with torch.no_grad():
            q_heads = self.q_network(state_tensor)[0]   # [K, A]
            q_mean = q_heads.mean(dim=0)
            q_std = q_heads.std(dim=0, unbiased=False)
            ucb_scores = q_mean + self.explore_hp.BETA * q_std
            action = ucb_scores.argmax().item()
            chosen_bonus = q_std[action].item()
            return action, chosen_bonus
